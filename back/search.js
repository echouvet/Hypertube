...
if (!empty(query))
{
	fetch('https://archive.org/services/search/v1/scrape?count=100&sorts=title%20desc&fields=identifier%2Cdownloads%2Ctitle%2Cmediatype%2Clanguage%2Cmonth%2Cweek%2Cpublicdate%2Cdescription%2Cavg_rating%2Cformat&q=title%3A(' + query + ')')
    	.then(res => res.json())
    	.then(function(json) {
			var i = 0;
			while (json.items[i])
